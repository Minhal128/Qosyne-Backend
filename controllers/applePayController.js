...
const APPLE_PAY_CERTIFICATE_PATH =
  process.env.APPLE_PAY_CERTIFICATE_PATH ||
  path.join(__dirname, "..", "..", "..", "apple_pay.csr");
const APPLE_PAY_PRIVATE_KEY_PATH =
  process.env.APPLE_PAY_PRIVATE_KEY_PATH ||
  path.join(__dirname, "..", "..", "..", "apple_pay.key");
...
const APPLE_PAY_MERCHANT_DOMAIN =
  process.env.APPLE_PAY_MERCHANT_DOMAIN || "qosyncefrontend.vercel.app";
...
if (BRAINTREE_MERCHANT_ID && BRAINTREE_PUBLIC_KEY && BRAINTREE_PRIVATE_KEY) {
  braintreeGateway = new braintree.BraintreeGateway({
    environment:
      braintree.Environment[BRAINTREE_ENVIRONMENT] ||
...
const validateMerchant = async (req, res) => {
  ...
  try {
    ...
    if (!APPLE_PAY_MERCHANT_ID) {
      console.error(
        "APPLE_PAY_MERCHANT_ID is not set in environment variables",
      );
      return res
        .status(500)
        .json({ error: "Apple Pay merchant ID not configured." });
    }

    if (!APPLE_PAY_MERCHANT_DOMAIN) {
      console.error(
        "APPLE_PAY_MERCHANT_DOMAIN is not set in environment variables",
      );
      return res
        .status(500)
        .json({ error: "Apple Pay merchant domain not configured." });
    }
    ...
    if (!fs.existsSync(APPLE_PAY_CERTIFICATE_PATH)) {
      console.error(
        `Apple Pay certificate not found at: ${APPLE_PAY_CERTIFICATE_PATH}`,
      );
      return res
        .status(500)
        .json({
          error:
            "Apple Pay certificate not found. Please ensure the certificate file is uploaded to the server.",
        });
    }
    ...
    if (APPLE_PAY_PRIVATE_KEY_ENV) {
      ...
      return res
        ...
        .json({
          error:
            "Apple Pay private key not configured. Provide APPLE_PAY_PRIVATE_KEY or APPLE_PAY_PRIVATE_KEY_PATH.",
        });
    }
...
const startApplePaySession = async (req, res) => {
  ...
  try {
    ...
    const currency =
      (payment && payment.total && payment.total.currency) || "USD";
    ...
    const paymentMethodNonce =
      ...
      payment.token.paymentData &&
      payment.token.paymentData.nonce;
    ...
    if (paymentMethodNonce && braintreeGateway) {
      ...
      if (result && result.success) {
        ...
        return res
          .status(200)
          .json({ success: true, transaction, braintree: btTransaction });
      }

      console.error("Braintree transaction failed", result);
      return res
        .status(500)
        .json({ error: "Braintree transaction failed", details: result });
    }
...
const handleApplePayCallback = async (req, res) => {
  try {
    console.log(
      "Received Apple Pay callback:",
      JSON.stringify(req.body, null, 2),
    );

    const { userId, paymentMethodToken, status } = req.body; // Assuming these are sent by the payment processor

    if (!userId || !paymentMethodToken) {
      console.error(
        "Missing userId or paymentMethodToken in Apple Pay callback",
      );
      return res.status(400).send("Missing required data");
    }

    // Here, you would typically validate the paymentMethodToken with your payment gateway
    // and then update your database to reflect the connected Apple Pay wallet for the user.

    // Example: Update a user's connected wallets in Prisma
    await prisma.connectedWallet.upsert({
      ...
      update: {
        ...
        status: status || "CONNECTED", // Assuming status can be provided or defaults to CONNECTED
        // You might also store other metadata from the callback
      },
      create: {
        userId: userId,
        provider: "APPLE_PAY",
        ...
        // Add any other necessary fields for a new connected wallet
      },
    });

    console.log(`Apple Pay successfully connected for user ${userId}`);

    // Redirect the user to the frontend base URL (configurable via FRONTEND_BASE_URL).
    // If FRONTEND_BASE_URL is not set, fall back to APPLE_PAY_MERCHANT_DOMAIN (with https),
    // and finally to a sensible default.
    const frontendBase =
      process.env.FRONTEND_BASE_URL ||
      (APPLE_PAY_MERCHANT_DOMAIN
        ? `https://${APPLE_PAY_MERCHANT_DOMAIN}`
        : "https://qosyncefrontend.vercel.app");

    // Redirect to the main website (302). You can change this to a specific success path if needed.
    res.redirect(302, frontendBase);
  } catch (error) {
    console.error("Error handling Apple Pay callback:", error);
    res.status(500).send("Failed to process Apple Pay callback");
  }
};
