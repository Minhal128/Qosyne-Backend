...
router.get(
  "/wallets",
  authMiddleware,
  tryCatch(walletIntegrationController.getUserWallets),
);
router.get(
  "/wallets/available-for-transfer",
  authMiddleware,
  tryCatch(walletIntegrationController.getAvailableWalletsForTransfer),
);
router.post(
  "/wallets/connect",
  authMiddleware,
  tryCatch(walletIntegrationController.connectWallet),
);
router.delete(
  "/wallets/:walletId/disconnect",
  authMiddleware,
  tryCatch(walletIntegrationController.disconnectWallet),
);
router.get(
  "/wallets/:walletId/balance",
  authMiddleware,
  tryCatch(walletIntegrationController.getWalletBalance),
);
router.post(
  "/wallets/:walletId/refresh",
  authMiddleware,
  tryCatch(walletIntegrationController.refreshWalletConnection),
);

// OAuth Routes
router.get(
  "/wallets/auth/:provider/url",
  authMiddleware,
  tryCatch(walletIntegrationController.getOAuthUrl),
);
router.post(
  "/wallets/auth/:provider/callback",
  authMiddleware,
  tryCatch(walletIntegrationController.handleOAuthCallback),
);
...
router.post(
  "/wise/oauth/init",
  authMiddleware,
  tryCatch(walletIntegrationController.initWiseOAuth),
);
router.get(
  "/wise/oauth/callback",
  tryCatch(walletIntegrationController.handleWiseOAuthCallback),
);

router.post(
  "/venmo/oauth/init",
  authMiddleware,
  tryCatch(walletIntegrationController.initVenmoOAuth),
);
router.get(
  "/venmo/oauth/callback",
  tryCatch(walletIntegrationController.handleVenmoOAuthCallback),
);

// Apple form_post callback (public endpoint) - accepts POST from Apple and redirects to SPA
router.post(
  "/wallets/auth/APPLEPAY/form_post",
  tryCatch(walletIntegrationController.handleAppleFormPost),
);

// Public test endpoint to simulate Apple's form_post (for device/browser testing).
// This endpoint accepts form-encoded POSTs and redirects the user-agent to the SPA callback
// with either the provided code/id_token/state or test values if omitted.
// Use this to verify the form_post flow without needing Apple Connect to hit your backend.
router.post(
  "/wallets/auth/APPLEPAY/test_form_post",
  express.urlencoded({ extended: true }),
  tryCatch(async (req, res) => {
    // Allow both form-encoded bodies and empty bodies for quick testing
    const { code, id_token, state } = req.body || {};
    console.log("🔧 Apple form_post test endpoint received body:", {
      code: !!code,
      id_token: !!id_token,
      state: !!state,
    });

    // Build redirect URL to SPA callback route (reuse same logic as handleAppleFormPost)
    const redirectBase =
      process.env.FRONTEND_URL || `${req.protocol}://${req.get("host")}`;
    const params = new URLSearchParams();
    params.set("code", code || "TEST_CODE");
    params.set("id_token", id_token || "TEST_ID_TOKEN");
    params.set("state", state || "TEST_STATE");

    const redirectUrl = `${redirectBase.replace(/\/$/, "")}/apple-pay/callback?${params.toString()}`;

    console.log(
      "🔧 Redirecting test form_post to SPA callback URL:",
      redirectUrl,
    );
...
// Mock connect (dev/testing only) - creates a fake Apple Pay wallet for the authenticated user
router.post(
  "/wallets/mock-connect",
  authMiddleware,
  tryCatch(walletIntegrationController.mockConnectApplePay),
);

// Shortcut 'complete' endpoint for testing without Apple device: create a mock Apple Pay
// connection for the authenticated user and redirect back to the frontend main page with
// a success flag. Useful when you don't have an iPhone or Apple redirects are failing.
router.post(
  "/wallets/auth/APPLEPAY/complete",
  authMiddleware,
  tryCatch(async (req, res) => {
    // Reuse the mock connect controller behavior
    const userId = req.user.userId;
    // Call the controller method directly
    const result = await walletIntegrationController.mockConnectApplePay(req, res);
    // After creation, redirect to frontend main with success flag
    const redirectBase = (process.env.FRONTEND_URL || `${req.protocol}://${req.get('host')}`).replace(/\/$/, '');
    const target = `${redirectBase}/?applepay_success=true`;
    console.log('🔧 ApplePay complete shortcut redirecting to:', target);
  ...
  tryCatch(async (req, res) => {
    ...
    try {
      ...
      res.json({
        ...
        message:
          "Venmo wallet connected successfully with your Braintree credentials!",
      });
    } catch (error) {
...
router.post(
  ...
  tryCatch(async (req, res) => {
    ...
    console.log(
      "API Token:",
      process.env.WISE_API_TOKEN ? "Present" : "Missing",
    );
    ...
    try {
      ...
      const personalProfile = profilesResponse.data.find(
        (p) => p.type === "personal",
      );
      const profileId = personalProfile
        ? personalProfile.id
      ...
      try {
        ...
        console.log(
          "Borderless accounts failed:",
          borderlessError.response?.data || borderlessError.message,
        );
      }
      ...
      try {
        ...
        console.log(
          "v4 balances failed:",
          v4Error.response?.data || v4Error.message,
        );
        ...
        try {
          ...
          console.log(
            "v1 balances also failed:",
            v1Error.response?.data || v1Error.message,
          );
      ...
      try {
        ...
        console.log(
          "Accounts failed:",
          accountsError.response?.data || accountsError.message,
        );
      }
      ...
      res.status(500).json({
        success: false,
        error:
          "Wise API Error: " + (error.response?.data?.message || error.message),
      });
    }
  }),
);
...
router.post(
  ...
  authMiddleware,
  tryCatch(async (req, res) => {
    const { amount, currency, recipientEmail, recipientName, recipientIban } =
      req.body;
    const userId = req.user.userId;
...
router.post(
  ...
  authMiddleware,
  tryCatch(walletIntegrationController.initiateTransfer),
);
router.get(
  "/transactions/:transactionId",
  authMiddleware,
  tryCatch(walletIntegrationController.getTransaction),
);
router.get(
  "/transactions",
  authMiddleware,
  tryCatch(walletIntegrationController.getUserTransactions),
);
router.post(
  "/transactions/:transactionId/cancel",
  authMiddleware,
  tryCatch(walletIntegrationController.cancelTransaction),
);
router.post(
  "/transactions/:transactionId/retry",
  authMiddleware,
  tryCatch(walletIntegrationController.retryTransaction),
);
router.post(
  "/transactions/estimate-fees",
  authMiddleware,
  tryCatch(walletIntegrationController.estimateTransferFees),
);
router.get(
  "/transactions/currencies/supported",
  authMiddleware,
  tryCatch(walletIntegrationController.getSupportedCurrencies),
);
...
router.post(
  "/qr/generate",
  authMiddleware,
  tryCatch(walletIntegrationController.generateQRCode),
);
router.post(
  "/qr/universal",
  authMiddleware,
  tryCatch(walletIntegrationController.generateUniversalQR),
);
router.post(
  "/qr/venmo",
  authMiddleware,
  tryCatch(walletIntegrationController.generateVenmoQR),
);
router.post(
  "/qr/connect-and-generate",
  authMiddleware,
  tryCatch(walletIntegrationController.connectWalletAndGenerateQR),
);
router.get(
  "/qr/:qrId/status",
  authMiddleware,
  tryCatch(walletIntegrationController.getQRStatus),
);
router.post(
  "/qr/scan/:qrId",
  tryCatch(walletIntegrationController.processQRScan),
);
router.post(
  "/qr/bank-deposit",
  authMiddleware,
  tryCatch(walletIntegrationController.generateBankDepositQR),
);
router.post(
  "/qr/wallet-connect",
  authMiddleware,
  tryCatch(walletIntegrationController.generateWalletConnectQR),
);
router.delete(
  "/qr/:qrId",
  authMiddleware,
  tryCatch(walletIntegrationController.deactivateQR),
);
router.get(
  "/qr",
  authMiddleware,
  tryCatch(walletIntegrationController.getUserQRCodes),
);

// Rapyd Integration Routes
router.get(
  "/rapyd/payment-methods/:country",
  authMiddleware,
  tryCatch(walletIntegrationController.getRapydPaymentMethods),
);
router.get(
  "/rapyd/exchange-rates",
  authMiddleware,
  tryCatch(walletIntegrationController.getRapydExchangeRate),
);
router.get(
  "/rapyd/countries",
  authMiddleware,
  tryCatch(walletIntegrationController.getRapydCountries),
);
router.get(
  "/rapyd/currencies",
  authMiddleware,
  tryCatch(walletIntegrationController.getRapydCurrencies),
);
